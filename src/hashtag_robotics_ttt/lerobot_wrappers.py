...
def _register_camera() -> None:
    ...
    import hashtag_robotics_ttt.config_avfoundation_uid  # noqa: F401
...
def _install_camera_quality_gate(module: Any) -> None:
    ...
    """
    from hashtag_robotics_ttt.avfoundation_uid import camera_incident_generation
...
def _install_ttt_demo_preset(strategy_class: type, preset: dict[str, Any]) -> None:
    ...
    def run_with_demo_preset(self: Any, ctx: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        print(
            f"Moving to demo episode {preset['episode_index']} start pose over 5 seconds...",
            flush=True,
        )
        try:
            ...
            print(f"Demo homing failed: {error}", flush=True)
        ...
        if worst_error > 3.0:
            ...
            print(
                f"Demo homing verification failed (maximum error {worst_error:.1f}°): {details}",
                flush=True,
            )
            _hold_current_ttt_pose_for_shutdown(ctx, tuple(target))
            events["stop_recording"] = True
        ...
        print(f"Demo home ready (maximum joint error {worst_error:.1f}°).", flush=True)
        _print_ttt_board(
            "Arrange the board exactly as seen by the TOP CAMERA:",
            preset["board_camera"],
        )
        _print_ttt_board("Equivalent robot/model orientation:", preset["board_robot"])
        print(
            "Place the requested piece in the source area. "
            "Ready: press Right Arrow or n; q cancels.",
            flush=True,
        )
        ...
        while not events["stop_recording"] and not ctx.runtime.shutdown_event.is_set():
            if events["rerecord_episode"]:
                events["rerecord_episode"] = False
                events["exit_early"] = False
                print("Press Right Arrow or n to start; q cancels.", flush=True)
